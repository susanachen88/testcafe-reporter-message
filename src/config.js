...
const defaultConfig = {
    webhookUrl: process.env.TESTCAFE_SLACK_WEBHOOK || 'https://hooks.slack.com/services/xxxxxxxx',
    channel: process.env.TESTCAFE_SLACK_CHANNEL,
    loggingLevel: process.env.TESTCAFE_SLACK_LOGGING_LEVEL || LoggingLevels.SUMMARY,
...
const loadReporterConfig = () => {
    ...
    try {
        ...
        return testCafeConfig.reporter.find(obj => obj.name === 'message');
    } catch (err) {
        return defaultConfig;
    }
};
